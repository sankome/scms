	...
	const content = createApp({
		template: `
			<div class="container" :class="{disabled: unfetching}">
				<h2 class="page-title">unlisted</h2>
				<p class="page-links">
					<a href="content-edit?unlisted">add</a>
				...
				<div v-if="contents && contents.length">
					...
					<ul class="paging">
						<li><a @click.prevent="unlist(1)" href="?page=1">&lt;&lt;</a></li>
						<li v-for="i in pages">
							<span v-if="i == current">{{i}}</span>
							<a v-else @click.prevent="unlist(i)" :href="'?page=' + i">{{i}}</a>
						</li>
						<li><a @click.prevent="unlist(lastPage)" :href="'?page=' + lastPage">&gt;&gt;</a></li>
					</ul>
				</div>
		...
		mounted() {
			this.ununlist(getParams().get("page"));
		},
		methods: {
			async ununlist(page) {
				...
				let unlist = await fetchGet(
					"contents/unlist",
					{limit: 5, offset: (page - 1) * 5, trim: 200},
				);
				this.contents = unlist.contents;
				
				let count = unlist.count;
			...
			async remove(id) {
				...
				if (removed.removed)this.ununlist();
				this.unfetching = false;
			},
		},
	}).mount(".main");
