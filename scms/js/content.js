	...
	const content = createApp({
		template: `
			<div class="container" :class="{disabled: unfetching}">
				...
				<div v-if="contents && contents.length">
					...
					<ul class="paging">
						<li><a @click.prevent="all(1)" href="?page=1">&lt;&lt;</a></li>
						<li v-for="i in pages">
							<span v-if="i == current">{{i}}</span>
							<a v-else @click.prevent="all(i)" :href="'?page=' + i">{{i}}</a>
						</li>
						<li><a @click.prevent="all(lastPage)" :href="'?page=' + lastPage">&gt;&gt;</a></li>
					</ul>
				</div>
		...
		methods: {
			async all(page) {
				...
				let all = await fetchGet(
					"contents/all",
					{limit: 5, offset: (page - 1) * 5, trim: 200},
				);
				this.contents = all.contents;
